...
HEALTHCHECK --interval=30s --timeout=5s --start-period=60s --retries=3 \
    CMD python -c "import os, urllib.request; host = os.getenv('DJANGO_ALLOWED_HOSTS', 'localhost').split(',')[0].strip() or 'localhost'; request = urllib.request.Request('http://127.0.0.1:' + os.getenv('PORT', '8000') + '/health/live/', headers={'Host': host}); urllib.request.urlopen(request, timeout=4)"
