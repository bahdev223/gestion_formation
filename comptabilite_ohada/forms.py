...
class EcritureForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if (
            exercice
            and date_ecriture
            and not exercice.date_debut <= date_ecriture <= exercice.date_fin
        ):
            self.add_error(
                "date_ecriture",
                "Cette date est hors de l'exercice sélectionné.",
            )
...
class BaseLigneFormSet(BaseInlineFormSet):
    def clean(self):
        ...
        for form in self.forms:
            ...
            if compte and (not compte.actif or not compte.est_mouvement):
                raise ValidationError(
                    "Toutes les lignes doivent utiliser un compte actif "
                    "mouvementable."
                )
            if (debit > 0) == (credit > 0):
                raise ValidationError(
        ...
        if count < 2:
            raise ValidationError(
                "Ajoutez au moins deux lignes comptables."
            )
        if total_debit <= 0 or total_debit != total_credit:
            raise ValidationError(
                f"L'écriture doit être équilibrée : débit {total_debit} / "
